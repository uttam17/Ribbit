...
public class FileHelper {
	...
	public static byte[] reduceImageForUpload(byte[] imageData) {
		Bitmap bitmap = ImageResizer.resizeImageMaintainAspectRatio(imageData, SHORT_SIDE_TARGET); //convert file to bitmap in memory
		
		ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
		bitmap.compress(Bitmap.CompressFormat.PNG, 100, outputStream);
		byte[] reducedData = outputStream.toByteArray(); // convert to reduced array
		try {
			outputStream.close();
		}
		catch (IOException e) {
	...
	public static String getFileName(Context context, Uri uri, String fileType) {
		...
		if (fileType.equals(ParseConstants.TYPE_IMAGE)) { //appends appropriate file extension
			fileName += "png";
		}
		else {
